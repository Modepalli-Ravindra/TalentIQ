...
import { Sparkles, Search, User as UserIcon, Briefcase, BarChart3, Command, ChevronDown, ShieldCheck, Lock, LogOut, LogIn, Bell, Settings, Bookmark, Calendar, Bot, FileText, Users, Shield, Menu, X } from 'lucide-react';
    ...
    <header className="sticky top-4 z-50 w-full max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 transition-all">
      <div className="w-full rounded-[50px] p-[1px] bg-gradient-to-r from-blue-500/60 via-purple-500/50 to-cyan-500/60 shadow-[0_0_25px_rgba(59,130,246,0.3)] hover:shadow-[0_0_35px_rgba(139,92,246,0.45)] transition-all overflow-hidden">
        <div className="w-full h-16 px-4 sm:px-6 flex items-center justify-between gap-2 rounded-[49px] bg-[#18181B]/90 backdrop-blur-2xl">
          ...
          {/* Navigation */}
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2 flex-1 min-w-0 overflow-x-auto hide-scrollbar px-2">
            {navItems.map((item) => {
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            {isAuthenticated && user ? (
              <div className="flex items-center gap-1.5 sm:gap-3 shrink-0 pl-2 border-l border-[#27272A]">
                <button
                ...
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-full transition-colors lg:hidden"
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-1.5 sm:gap-2 hover:bg-white/5 p-1 rounded-full sm:rounded-l-full sm:pr-3 transition-colors focus:outline-none"
                  ...
                  {showUserMenu && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-[#18181B] border border-[#27272A] rounded-2xl shadow-xl py-2 z-20 animate-in fade-in slide-in-from-top-2 duration-200">
                        <div className="px-4 py-2 border-b border-[#27272A] sm:hidden">
                          <p className="text-xs font-bold text-white">{user.name}</p>
                          <p className="text-[10px] text-blue-400 font-mono capitalize">{user.role}</p>
                        </div>
                        <button
                          onClick={() => {
                          ...
                          className="w-full text-left px-4 py-2 text-xs text-gray-300 hover:text-white hover:bg-white/5 transition-colors flex items-center gap-2"
                        >
                        ...
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-xs text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-colors flex items-center gap-2 border-t border-[#27272A]"
                        >
                ...
                <button
                  onClick={() => logout()}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 hover:border-rose-500/50 rounded-full text-xs font-semibold transition-all shrink-0"
                  title="Sign Out"
                >
        ...
        {showMobileMenu && (
          <div className="absolute top-20 left-4 right-4 bg-[#18181B] border border-[#27272A] rounded-2xl shadow-xl p-3 z-30 flex flex-col gap-1 lg:hidden animate-in fade-in slide-in-from-top-4 duration-200">
            {navItems.map((item) => {
              const isActive = currentView === item.id;
              const isLocked = !item.public && !isAuthenticated;
              return (
                <button
                  ...
                  className={`w-full px-4 py-2.5 rounded-xl text-xs font-semibold transition-all flex items-center justify-between ${
                    isActive
            ...
            {isAuthenticated && (
              <button
                ...
                className="w-full px-4 py-2.5 mt-2 rounded-xl text-xs font-semibold text-rose-400 bg-rose-500/10 border border-rose-500/30 hover:bg-rose-500/20 hover:border-rose-500/50 transition-all flex items-center gap-2"
              >
                <LogOut className="w-4 h-4" />
                <span>Sign Out</span>
              </button>
